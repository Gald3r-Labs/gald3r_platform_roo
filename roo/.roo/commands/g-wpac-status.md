...
CONSTRAINT OVERLAP
  ...
  Action: [none needed | run @g-wpac-sync to inherit N missing constraints]
────────────────────────────────────────
```
...
### 8. Report
Display the full status block above. If CONFLICTs exist, recommend running `@g-wpac-read` immediately.
