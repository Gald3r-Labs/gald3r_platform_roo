...
### Mid-Flight Course Correction (`/steer` + `/queue`) in swarm mode (T969)

The `@g-steer` / `@g-queue` mechanism is **per-worktree** (see "Mid-Flight Course Correction" in
`@g-go-code`). In swarm mode each bucket owns its own coding worktree, so steer/queue target a
bucket: `@g-steer T{id} ...` and `@g-queue T{id} ...` resolve to the worktree holding `T{id}`. Each
bucket agent runs its own steer poll (step b2.5) and queue drain (step 7d) independently against its
own worktree root — the coordinator does not aggregate steers across buckets.
